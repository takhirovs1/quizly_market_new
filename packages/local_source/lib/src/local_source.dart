...
import 'utils/preference_dao_extension.dart';
...
/// {@endtemplate}
@immutable
final class LocalSource extends PreferenceDao with UserDataSourceImpl, SettingsDataSourceImpl {
  ...
  static Future<LocalSource> _init() async {
    ...
    // encrypted entry keys
    final encryptedEntryKeys = <String>{instance._accessTokenKey.key, instance._refreshTokenKey.key};

    // all entries from the local source
    final entries = [
      // settings data
  ...
  Future<void> clearAllPersonalData() async {
    cache.clear();
    await clearUserData();
  ...
  Future<void> clearAll() async {
    cache.clear();
    await Future.wait<void>([clearUserData()]);
